//TC1157: Verify that Live Assist global contextual variables from agent GUI selection added to web api call response (outcome return)
//-------------------------------------------------------

describe('TC1157: Verify that Live Assist global contextual variables from agent GUI selection added to web api call response (outcome return)', function() {
...
//-------------------------------------------------------
    it('TC1157: Verify that Live Assist global contextual variables from agent GUI selection added to web api call response (outcome return)', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    ...
                    element1.sendKeys('TA0002').then(function(){
...
//                    element1 = ptor.findElements(protractor.By.repeater('checkboxRow in checkboxRows')).then(function(arr_1) {
                    //                       element2 = ptor.findElements(protractor.By.repeater('checkbox in arr_1')).then(function (arr) {
                    //                         arr[0].evaluate('checkbox.id').then(function (checkbox_id) {  //#3 is set for 'operator', but if id# changed, update at here
                    var chk1 = 'ENGLISH' + 'CheckBox';
                    element3 = ptor.element(protractor.By.id(chk1));
                    element3.click().then(function () {
                        output_msg = '\ncheckbox selection: ' + chk1;
                        output_string += output_msg;
                        logger.info(output_msg);
                        //
                        var result = [chk1, 0];
...
//                    element1 = ptor.findElements(protractor.By.repeater('checkboxRow in checkboxRows')).then(function(arr_1) {
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[18][1]).to.match(/<inputInteractionStepResult>/)
                        ...
                        .and.to.match(/<\/intent>/)
                        .and.to.match(/<globals>ENGLISH<\/globals>/)
                        .and.to.match(/<globals>MALE<\/globals>/)
