//TC1162: Verify that Live Assist agent GUI keyboard input CTRL+END to sign out from idle state
//-------------------------------------------------------
describe('TC1162: Verify that Live Assist agent GUI keyboard input CTRL+END to sign out from idle state', function() {
//QA test setup ----------------------------------------------
    var case_ID = 1162;
...
//-------------------------------------------------------
    it('TC1162: Verify that Live Assist agent GUI keyboard input CTRL+END to sign out from idle state', function(done) {
        //trace HTTP call
        //nock.recorder.rec();
        ...
        async.series([
                ...
                function(callback){
                    ...
                    ptor.driver.wait(function(){
                        ...
                        if(b){
                            ...
                            //
                            ptor.getLocationAbsUrl().then(function(url){
                                output_msg = '\nAgent Sign out page and button found. Current location url: ' + url;
                                output_string += output_msg;
                                console.log(output_msg);
                                //
                                var result = [output_msg, 0];
                ...
                function(callback){
                    //wait for next page ############
                    ptor.driver.wait(function(){
                        return ptor.isElementPresent(protractor.By.id('agentSignInButton'));
                    }, 12000).then(function(b){
                        if(b){
                            output_msg = '\nFound the new agent web page with specific className';//for debugging
                            output_string += output_msg;
                            ...
                            //
                            ptor.getLocationAbsUrl().then(function(url){
                                output_msg = '\nAgent sign in page and button found. Current location url: ' + url;
                                output_string += output_msg;
                                console.log(output_msg);
                                //
                                var result = [output_msg, 0];
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[7][0]).to.match(/Agent Sign out page and button found/); //login page (all point to /app)
                    expect(result[8][0]).to.match(/Sign out by CTRL \+ END/); //login page (all point to /app)
                    expect(result[9][0]).to.match(/Agent sign in page and button found/); //login page (all point to /app)
