//TC1198: Verify that Live Assist MRCP API with resultMediaType for NLSML-> agent outcome selection -> agent GUI response & summary
//-------------------------------------------------------
describe('TC1198: Verify that Live Assist MRCP API with resultMediaType for NLSML-> agent outcome selection -> agent GUI response & summary', function() {
...
//global env settings
    ...
    this.timeout(90000); //set timeout for test case
...
//-------------------------------------------------------
    it('TC1198: Verify that Live Assist MRCP API with resultMediaType for NLSML -> agent outcome selection -> agent GUI response & summary', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    ...
                    test_client.post(testAPIURL + '/startSession', testBodyParam, function(err, res, body){
                        if(err){
                ...
                function(callback){
                    ...
                    test_client.post(testAPIURL + '/startRecognition ', testBodyParam, function(err, res, body){
                        if(err){
                ...
                function(callback){
                    ...
                    var testBodyParam = JSON.stringify({
                        ...
                        resultMediaType:"application/x-vnd.speechworks.emma+xml",
                    ...
                    test_client.post(testAPIURL + '/sendRecognitionResult  ', testBodyParam, function(err, res, body){
                        if(err){
                            err_msg='Error found from http post request on /startRecognition' + err;
                            err_array.push(err_msg);
                            logger.info(err_msg, err);
                ...
                function(callback){
                    ...
                    ptor.driver.wait(function(){
                        ...
                        //return ptor.isElementPresent(protractor.By.id('signOutButton'));
                        //return ptor.isElementPresent(protractor.By.input('playbackRate'));
                    }, 12000).then(function(b){
                        if(b){
                            output_msg = '\nFound the new agent web page with specific className';//for debugging
                            output_string += output_msg;
                        ...
                        async.whilst(
                            ...
                            function(cb){
                                arr[i].evaluate('variable.id').then(function(var_id){
                                    arr[i].evaluate('variable.name').then(function(var_name){
                                        ...
                                        logger.info(output_msg);
                                        //
                                        if(!arr[i].evaluate('variable.confirmedByAgent')){
                                            element2 = ptor.element(protractor.By.xpath("//input[@type='checkbox']"));
                                            element2.click().then(function(){
                                                //element2.sendKeys('QA TEST1').then(function(){
                                                //
                                                i++;
                                                setTimeout(cb, 20);
                                            });
...
//old:                ptor.element(protractor.By.binding('lastResponse.intent.id'))
                    ptor.element(protractor.By.binding('lastResponse.intent.value')).getText().then(function(txt){
...
//old:                ptor.element.all(protractor.By.repeater('variable in lastResponse.intent.variables')).then(function(arr){
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[2][0]).to.match(/\/liveassist\/app/);    //sign in page
                    ...
                    expect(result[16][0]).to.match(/\/liveassist\/app/); //
                    expect(result[17][0]).to.match(/Waive fee/);                     //intent check

                    //agent selection output check
                    expect(result[22][0]).to.match(/WAIVE_FEE_REQUEST/); //intent id
                    //Note: Just 'WAVI_FEE_REQUEST intent click, no variables, so no valid following variables check anymore
                    expect(result[23][0][0]).to.match(/BILLING_MONTH/);
                    expect(result[23][1][0]).to.match(/2014-01/);
                    //expect(result[18][0][1]).to.match(/LATE_PAYMENT_REASON/);
