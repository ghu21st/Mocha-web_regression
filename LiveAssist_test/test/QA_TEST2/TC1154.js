//TC1154: Verify that Live Assist sample app agent with update contact info intent and custom values: (account) -> (update contact info) -> (first_name = QA_test1) -> (last_name = QA_test2)
//-------------------------------------------------------
describe('TC1154: Verify that Live Assist sample app agent with update contact info intent and custom values: (account) -> (update contact info) -> (first_name = QA_test1) -> (last_name = QA_test2)', function() {
...
//-------------------------------------------------------
    it('TC1154: Verify that Live Assist sample app agent with update contact info intent and custom values: (account) -> (update contact info) -> (first_name = QA_test1) -> (last_name = QA_test2)', function(done) {
                                            ...
                                            arr[1].evaluate('variable.id').then(function(var_id) {
                                                arr[1].evaluate('variable.name').then(function (var_name) {
                                                    ...
                                                    element2.click().then(function(){
                                                        element3 = ptor.element.all(protractor.By.id(var_id)).get(0);
                                                        element3.clear().then(function() {
                                                            element3.sendKeys('QA_TEST2').then(function () {
                                                                //
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[17][1]).to.match(/\"completionCause\":\"success\"/)
                        ...
                        .and.to.match(/\"id\":\"FIRST_NAME\"/)
                        .and.to.match(/\"value\":\"QA_TEST1\"/)
                        .and.to.match(/\"id\":\"LAST_NAME\"/)
                        .and.to.match(/\"value\":\"QA_TEST2\"/)
