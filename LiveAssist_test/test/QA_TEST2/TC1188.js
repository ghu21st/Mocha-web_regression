//TC1188: Verify that Live Assist popup calendar when agent selecting date via waive fee intent (yyyy-mm) with basic keyboard controls
//US7411 As a Live Assist application developer I want to show a calendar popup to agents when selecting a date
// -------------------------------------------------------
describe('TC1188: Verify that Live Assist popup calendar when agent selecting date via waive fee intent with basic Arrow - Enter keyboard controls', function() {
...
//-------------------------------------------------------
    it('TC1188: Verify that Live Assist popup calendar when agent selecting date via waive fee intent (yyyy-mm) with basic keyboard controls', function(done) {
  ...
  */
                    ...
                    ptor.element.all(protractor.By.repeater('variable in selectedIntents[selectedIntents.length - 1].variables')).then(function(arr){
                        ...
                        arr[0].evaluate('variable.id').then(function(var_id) {
                            arr[0].evaluate('variable.name').then(function (var_name) {
                                ...
                                element2.sendKeys(protractor.Key.SPACE).then(function(){       //trigger the date
                                    var chk1 = var_id + 'Button';
                                    element3 = ptor.element(protractor.By.id(chk1));
                                    element3.sendKeys(protractor.Key.ENTER).then(function(){          //trigger calendar popup calendar
            ...
            function(err, result){
                ...
                try{
                    ...
                    //get current year and Feb with yyyy-mm format for this case testing
                    Date.prototype.yyyy_Feb = function() {
                        var yyyy = this.getFullYear().toString();
                        return yyyy + '-' + '02'; // padding
                    };

                    d = new Date();
                    var current_year_Feb = d.yyyy_Feb();
                    logger.info('\ncurrent date year-month plus 1 month for testing: ' + current_year_Feb);
                    //
                    expect(result[17][1]).to.match(/\"completionCause\":\"success\"/)
                        ...
                        .and.to.match(/\"value\":\"2014-02\"/)
                        .and.to.match(/\"state\":\"useLiveAssistResult\"/);

                    expect(result[17][1]).to.have.string(current_year_Feb);  //verify if contain the basic keyboard control selected year-month
