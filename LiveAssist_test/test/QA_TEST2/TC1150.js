//TC1150: Verify that Live Assist sample app agent select flight intent with customized city outcome value selection not from the default list: (Flight) -> (Booking) -> (QA_departure_city) -> (New York)
//-------------------------------------------------------

describe('TC1150: Verify that Live Assist sample app agent select flight intent with customized city outcome value selection not from the default list: (Flight) -> (Booking) -> (QA_departure_city) -> (New York)', function() {
...
//-------------------------------------------------------
    it('TC1150: Verify that Live Assist sample app agent select flight intent with customized city outcome value selection not from the default list: (Flight) -> (Booking) -> (QA_departure_city) -> (New York)', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    ...
                    element1 = ptor.findElements(protractor.By.repeater('intent in intents').column('name')).then(function(arr){
                        arr[2].getText().then(function(intent_name){
                            ...
                            element2.click().then(function(){
                                ptor.findElements(protractor.By.repeater('intent in selectedIntents[0].members').column('name')).then(function(arr){
                                    arr[0].getText('intent.name').then(function(intent_name){
                            ...
                            arr[0].evaluate('variable.name').then(function (var_name) {
                                ...
                                element2.click().then(function(){
                                    ...
                                    element3.clear().then(function(){
                                        element3.sendKeys('QA city').then(function () {
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[18][1]).to.match(/\"completionCause\":\"success\"/)
                        ...
                        .and.to.match(/\"value\":\"QA city\"/)
                        .and.to.match(/\"id\":\"ARRIVAL_CITY\"/)
                        .and.to.match(/\"value\":\"New York\"/)
                        .and.to.match(/\"state\":\"useLiveAssistResult\"/);
                    /*
                     "variables":[{"id":"DEPARTURE_CITY","value":"QA TEST1"},{"id":"ARRIVAL_CITY","value":"QA_TEST2"},{"id":"DEPARTURE_DAY","value":"2014-12-21"},{"id":"DEPARTURE_TIME","value":"10:30"}]}
                     */
