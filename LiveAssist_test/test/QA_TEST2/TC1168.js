//TC1168: Verify that Live Assist MRCP api call with agent global contextual variables selected as agent outcome
//-------------------------------------------------------

describe('TC1168: Verify that Live Assist MRCP api call with agent global contextual variables selected as agent outcome', function() {
    ...
    var test_client = {
        ...
        post2: function(url, bodyParam, cb){
            request.post({
                ...
                ,function (err, res, body) {
                    if(err){
                        err_msg='\ntest client http post2 request found error: ' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        cb(err, res, body);
                    }else{
        ...
        delete: function(url, cb){
            request.del(url, function(err, res, body){
                if(err){
                    err_msg='\ntest client http delete request found error: ' + err;
                    err_array.push(err_msg);
                    console.log(err_msg, err);
                    cb(err, res, body);
                }
...
//-------------------------------------------------------
    it('TC1168: Verify that Live Assist MRCP api call with agent global contextual variables selected as agent outcome', function(done) {
        ...
        var testAPIURL = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/rest/mrcpapp';
        ...
        async.series([
                ...
                //################# App test driver --- MRCP app API to trigger agent escalation
                function(callback){
                    //
...
//ex: varName=Nuance-MrcpApiCallSteering,varVersion=1.0,ani=sip:5149047800@10.3.41.105:35020;
// tag=0a032969_00004d83_538399a1_000b_0001,dnis=sip:4641@127.0.0.1:5066,aai=null,externalSessionID=0a032969_00004d83_538399a1_000b_0001,
// audioRelayServerUrl=http://10.3.41.105:8081,applicationConfiguration=3,canStreamAudio=true,
// currentInteractionId=null,interactions=[],shouldEscalate=false,escalated=false,
// sessionStats=SessionStats[{CREATE=1401133560554, START_SESSION=1401133560554}],viewName=null
                    test_client.post2(testAPIURL + '/startSession', testBodyParam, function(err, res, body){
                        if(err){
                            err_msg='Error found from http post2 request on /startSession' + err;
                            err_array.push(err_msg);
                            console.log(err_msg, err);
                            callback(err, 9);
                        }
                        ...
                        output_string += '\n\nDebug: send post2 request \'\/startSession\' done\n\n';//for debugging
                        output_string += '\npost2 test body param: ' + testBodyParam; //for debugging
                        output_string += '\nResponse: ' + res;//for debugging
...
// sessionStats=SessionStats[{CREATE=1401133560554, START_SESSION=1401133560554}],viewName=null
                    ...
                    test_client.post2(testAPIURL + '/startRecognition ', testBodyParam, function(err, res, body){
                        if(err){
                            err_msg='Error found from http post2 request on /startRecognition' + err;
                            err_array.push(err_msg);
                            console.log(err_msg, err);
                            callback(err, 11);
                        }
                        ...
                        output_string += '\n\nDebug: send post2 request \'\/startRecognition \' done\n\n';//for debugging
                        output_string += '\npost2 test body param: ' + testBodyParam; //for debugging
                        output_string += '\nResponse: ' + res;//for debugging
                        output_string += '\nBody: ' + body_ret;//for debugging
                        ...
                        msg = msg + 'sendAudio from /startRecognition is: ' + sendAudio + '\n';
                        console.log(msg);
                        output_string += msg;
                ...
                function(callback){
                    ...
                    var testBodyParam = JSON.stringify({
                        ...
                        utterance:"http://mtl-bl1-12-vm02:90/Nuance/callLogs/MrcpApiCallSteering/2014/05May/26/15/NUAN-44-33-mtl-bl1-12-vm02-0a032969_00004d83_538399a1_000b_0001-utt001-SAVEWAVEFORM.wav",
                        ...
                        recognitionResult:""
                    });

                    test_client.post2(testAPIURL + '/sendRecognitionResult  ', testBodyParam, function(err, res, body){
                        if(err){
                            err_msg='Error found from http post2 request on /startRecognition' + err;
                            err_array.push(err_msg);
                            console.log(err_msg, err);
                        ...
                        output_string += '\n\nDebug: send post2 request \'\/sendRecognitionResult  \' done\n\n';//for debugging
                        output_string += '\npost2 test body param: ' + testBodyParam; //for debugging
                        output_string += '\nResponse: ' + res;//for debugging
                        output_string += '\nBody: ' + body_ret;//for debugging
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[14][0].statusCode).to.eql(200);
                    expect(result[14][1].completionCause).to.match(/success/);
                    expect(result[14][1].state).to.match(/useLiveAssistResult/);

                    expect(result[35][1]).to.match(/\"recognitionResultProvided\":true/)
                        .and.to.match(/\"completionCause\":\"success\"/)
                        .and.to.match(/\"agentUsername\":\"TA.+\"/)
                    ...
                    /*
                     "outcome":{"intent":{"id":"INTENT","value":"WAIVE_FEE_REQUEST"},"variables":[],"globals":["ENGLISH","SPANISH","MALE","FEMALE","TOO_NOISY",
                     "UNINTENDED_INPUT","NO_MATCHING_INTENT","NO_INPUT_FROM_USER","REPROMPT_REQUIRED","ESCALATE_TO_AGENT"]}
                   */
