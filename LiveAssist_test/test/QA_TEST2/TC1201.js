//TC1201: Verify that Live Assist MRCP API with NLSML format input -> agent intent selection with global and variables -> generate agent outcome response via debug api check
//-------------------------------------------------------
describe('TC1201: Verify that Live Assist MRCP API with NLSML format input -> agent intent selection with global and variables -> generate agent outcome response via debug api check', function() {
...
//-------------------------------------------------------
    it('TC1201: Verify that Live Assist MRCP API with NLSML format input -> agent intent selection with global and variables -> generate agent outcome response via debug api check', function(done) {
...
// sessionStats=SessionStats[{CREATE=1401133560554, START_SESSION=1401133560554}],viewName=null
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[2][0]).to.match(/https:.+liveassist\/app/); //default sign in page (all point to /app)
                    ...
                    expect(result[13][1].completionCause).to.match(/success/);
                    expect(result[13][1].state).to.match(/useLiveAssistResult/);
                    ...
                    expect(result[23][1]).to.match(/\"applicationID\":\"MrcpApiCallSteering\"/)
                        ...
                        .and.to.match(/\"value\":\"2014-01\"/)
                        .and.to.match(/\"topInterpretation\":/)
                        .and.to.match(/\"confidence\":\"0.4\"/)
                        .and.to.match(/\"grammarRef\":\"session:0x00000001\"/)
                        .and.to.match(/\"globals\":/)
                        .and.to.match(/\"ENGLISH\"/)
                        .and.to.match(/\"MALE\"/);

                    // .and.to.match(/\"id\":\"LATE_PAYMENT_REASON\"/)
                    // .and.to.match(/\"value\":null/)
