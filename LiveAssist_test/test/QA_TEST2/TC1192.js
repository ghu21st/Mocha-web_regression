//TC1192: Verify that Live Assist popup calendar when agent selecting date via book flight intent daily view with basic End-PageUp-PageDown keyboard controls
//-------------------------------------------------------
describe('TC1192: Verify that Live Assist popup calendar when agent selecting date via book flight intent daily view with basic End-PageUp-PageDown keyboard controls', function() {
//QA test setup ----------------------------------------------
    var case_ID = 1192;
...
//-------------------------------------------------------
    it('TC1192: Verify that Live Assist popup calendar when agent selecting date via book flight intent daily view with basic End-PageUp-PageDown keyboard controls', function(done) {
        ...
        async.series([
                                ...
                                ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.ENTER).then(function () {
                                    ...
                                    ptor.driver.getAllWindowHandles().then(function (handles) {
                                        ...
                                        ptor.driver.switchTo().window(handles[0]).then(function () {
                                            ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.END).then(function () { //HOME key, first month of current year
                                                ptor.driver.sleep(500);
            ...
            function(err, result){
                ...
                try{
                    ...
                    Date.prototype.e_yyyymm = function() {
                        var yyyy = this.getFullYear().toString();
                        var mm = (this.getMonth() + 1 + 1).toString(); // getMonth() is zero-based so plus 1, also for this case plus 1 more
                        return yyyy + '-' + (mm[1]?mm:"0"+mm[0]); //
                    };

                    d = new Date();
                    var expected_year_month = d.e_yyyymm();
                    days = new Date(d.getFullYear(), d.getMonth() + 2, 0).getDate();    //note: as for this case, getMonth() need to be added 2
                    var expected_year_month_day = expected_year_month + '-' + days.toString();
