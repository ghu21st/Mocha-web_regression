//TC1151: Verify that Live Assist sample app agent select flight intent with customized date and time outcome value: (Flight) -> (Booking) -> (QA_TEST1) -> (Seattle) -> (2014-12-21) -> (10:30)
//-------------------------------------------------------
describe('TC1151: Verify that Live Assist sample app agent select flight intent with customized date and time outcome value: (Flight) -> (Booking) -> (QA_TEST1) -> (Seattle) -> (2014-12-21) -> (10:30)', function() {
//QA test setup ----------------------------------------------
    var case_ID = 1151;
...
//-------------------------------------------------------
    it('TC1151: Verify that Live Assist sample app agent select flight intent with customized date and time outcome value: (Flight) -> (Booking) -> (QA_TEST1) -> (Seattle) -> (2014-12-21) -> (10:30)', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    //Select agent outcome intent  ##############
                    //element1 = ptor.findElements(protractor.By.repeater('intent in intents').column('name')).then(function(arr){
                    ptor.element.all(protractor.By.repeater('intent in intents')).then(function(arr){
                        arr[2].evaluate('intent.name').then(function(intent_name){
                    ...
                    ptor.element.all(protractor.By.repeater('variable in selectedIntents[selectedIntents.length - 1].variables')).then(function(arr){
                        ...
                        arr[0].evaluate('variable.id').then(function(var_id) {
                            arr[0].evaluate('variable.name').then(function (var_name) {
                                ...
                                element2.click().then(function(){
                                    element3 = ptor.element(protractor.By.id(var_id));
                                    element3.clear().then(function(){
                                        element3.sendKeys('QA Test').then(function () {

                                            //arr[1] - arrival city
                                            arr[1].evaluate('variable.id').then(function(var_id) {
                                                arr[1].evaluate('variable.value').then(function (var_value) {
                                                    var_id_res.push(var_id);
                                                    var_name_res.push(var_value);

                                                    //output selection to console
                                                    output_msg = '\nSelected variable id: ' + var_id;
                                                    output_msg += '\nDefault variable value before selection: ' + var_value;
                                                    output_string += output_msg;
                                                    console.log(output_msg);
                                                    ...
                                                    element2 = ptor.element(protractor.By.id(chk1));
                                                    element2.click().then(function(){
                                                        element3 = ptor.element.all(protractor.By.tagName('option')).then(function(options){
                                                            options[4].click().then(function(){
                                                                //
                                                             //   var_value = element3.getAttribute("value");
                                                              //  var_value_res.push(var_value);
                                                               // output_msg = '\nSelected variable value: ' + var_value;
                                                              //  output_string += output_msg;
                                                             //   console.log(output_msg);

                                                                //arr[2] - departure day
                                                                arr[2].evaluate('variable.id').then(function(var_id) {
                                                                    arr[2].evaluate('variable.name').then(function (var_name) {
                                                                        var_id_res.push(var_id);
                                                                        var_name_res.push(var_name);
                                                                        ...
                                                                        element2.click().then(function(){
                                                                            ...
                                                                            element3.clear().then(function() {
                                                                                element3.sendKeys('2014-12-21').then(function () {
                                                                                    //
                                                                                    var_value = element3.getAttribute("value");
                                                                                    var_value_res.push(var_value);
                                                                                    output_msg = '\nSelected variable value: ' + var_value;
                                                                                    ...
                                                                                    //arr[3] - departure time
                                                                                    arr[3].evaluate('variable.id').then(function(var_id) {
                                                                                        arr[3].evaluate('variable.name').then(function (var_name) {
                                                                                            var_id_res.push(var_id);
                                                                                            var_name_res.push(var_name);
                                                                                            ...
                                                                                            element2.click().then(function(){
                                                                                                element3 = ptor.element.all(protractor.By.id(var_id)).get(0);
                                                                                                element3.clear().then(function() {
                                                                                                    element3.sendKeys('10:30').then(function () {
                                                                                                        var_value = element3.getAttribute("value");
                                                                                                        var_value_res.push(var_value);
                                                                                                        output_msg = '\nSelected variable value: ' + var_value;
                                                                                                        output_string += output_msg;
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[9][1].completionCause).to.match(/null/);

                    expect(result[17][1]).to.match(/\"completionCause\":\"success\"/)
                        .and.to.match(/\"outcome\":/)
                        .and.to.match(/\"intent\":/)
                        .and.to.match(/\"id\":\"INTENT\"/)
                        .and.to.match(/\"value\":\"BOOK_FLIGHT\"/)
                        .and.to.match(/\"variables\":/)
                        .and.to.match(/\"id\":\"DEPARTURE_CITY\"/)
                        .and.to.match(/\"value\":\"QA Test\"/)
                        .and.to.match(/\"id\":\"ARRIVAL_CITY\"/)
                        .and.to.match(/\"value\":\"Seattle\"/)
                        .and.to.match(/\"id\":\"DEPARTURE_DAY\"/)
                        .and.to.match(/\"value\":\"2014\-12\-21\"/)
                        .and.to.match(/\"id\":\"DEPARTURE_TIME\"/)
                        .and.to.match(/\"value\":\"10:30\"/)
