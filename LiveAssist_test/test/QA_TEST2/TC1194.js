//TC1194: Verify that Live Assist simple API multiple interactions can re-enter the same context and escalate call to agent GUI properly
//DE4621 Using the Live Assist Simple API, cannot re-enter same context
//-------------------------------------------------------

describe('TC1194: Verify that Live Assist simple API multiple interactions can re-enter the same context and escalate call to agent GUI properly', function() {
...
//-------------------------------------------------------
    it('TC1194: Verify that Live Assist simple API multiple interactions can re-enter the same context and escalate call to agent GUI properly', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    ...
                    var testBodyParam = JSON.stringify({
                        ...
                        "externalSessionID":"1234",
                        "configurationName":"mainmenu",
                        "userInputText":"Test interaction 1"
                    });
                ...
                function(callback){
                    //
                    ptor.element(protractor.By.binding('interaction.text')).getText().then(function (text4) {
                        //
                ...
                //########## app test driver --- send 2nd interaction for this case testing!
                function(callback){
                    //
                    var testBodyParam = JSON.stringify({
                        "configurationName":"mainmenu",
                        "sessionID": sessionID,
                        "userInputText":"Test interaction 2"
                    });
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[11][0]).to.match(/Test interaction 1/);

                        //check returned agent outcome
                    expect(result[15][1]).to.match(/\"completionCause\":\"success\"/)
                    ...
                    //2nd interaction check
                    expect(result[17][0].statusCode).to.eql(200);
                    expect(result[17][1].state).to.match(/agentPending/);
                    expect(result[17][1].completionCause).to.match(/null/);

                        //check user input text
                    expect(result[19][0]).to.match(/Test interaction 2/);

                        //check returned agent outcome
                    expect(result[25][1]).to.match(/\"completionCause\":\"success\"/)
                        .and.to.match(/\"outcome\":/)
                        .and.to.match(/\"intent\":/)
                        .and.to.match(/\"id\":\"INTENT\"/)
                        .and.to.match(/\"value\":\"EXPLAIN_FEE_REQUEST\"/)
                        .and.to.match(/\"state\":\"useLiveAssistResult\"/);
                    /*
