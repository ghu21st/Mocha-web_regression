//TC1139: Verify that Live Assist simple IVR API method /inputInteractionStep -> agent outcome selection -> agent GUI response & selection summary
//-------------------------------------------------------
describe('TC1139: Verify that Live Assist simple IVR API method /inputInteractionStep -> agent outcome selection -> agent GUI response & selection summary', function() {
...
//-------------------------------------------------------
    it('TC1139: Verify that Live Assist simple IVR API method /inputInteractionStep -> agent outcome selection -> agent GUI response & selection summary', function(done) {
...
/*            function(callback){
  ...
  */
			...
			//wait for 1 sec
            function(callback){
                //special timeout here to make sure agent signed in 
                setTimeout(function(){
                    output_msg = '\nwait 1 second for server\n';
                    output_string += output_msg;
                    logger.info(output_msg);
            ...
            function(callback){
                //ivr call steering app: dnis=4627, app context = 'othermenu', confidence value = 0.4 (<0.5 default)
                ...
                //var testBodyParam = "ani=sip:5145551234@10.3.53.192:5064&dnis=sip:4627@mtl-da53&externalSessionID=0a0325e7_000011c2_5307b580_00d4_0001&sessionID=&configurationName=othermenu&completionCause=success&recognitionResult=%3C%3Fxml%20version%3D%271.0%27%3F%3E%3Cemma%3Aemma%20version%3D%271.0%27%20xmlns%3Aemma%3D%27http%3A%2F%2Fwww.w3.org%2FTR%2F2007%2FCR-emma-20071211%27%20xmlns%3Anuance%3D%27http%3A%2F%2Fnr10.nuance.com%2Femma%27%3E%3Cemma%3Agrammar%20id%3D%27grammar_1%27%20ref%3D%27session%3Ahttp%3A%2F%2Fmt-ray-vm03%3A8080%2Fliveassist%2Fdata%2Fvxmldemo%2Fivrapi%2Fcallsteering%2Fmainmenu.grxml%20-1%20-1%2010000%27%2F%3E%3Cemma%3Ainterpretation%20id%3D%27interp_1%27%20emma%3Auninterpreted%3D%27true%27%20emma%3Aconfidence%3D%270.3%27%20emma%3Agrammar-ref%3D%27grammar_1%27%20emma%3Atokens%3D%27operator%27%20emma%3Aduration%3D%277100%27%20nuance%3Aeos-silence%3D%271173%27%2F%3E%3C%2Femma%3Aemma%3E&utterance=http://mtl-da53:90/Nuance/callLogs/Test1/2014/02February/21/15/NUAN-22-24-mtl-da53-0a0325e7_000011c2_5307b580_00d4_0001-utt001-SAVEWAVEFORM.wav";
            ...
            function(callback){
                //check intent group name result
                ptor.element(protractor.By.binding('lastResponse.intent.value'))
                    .getText().then(function(txt){
                        output_msg = '\nSelected intent group name: ' + txt;
                        output_string += output_msg;
                        logger.info(output_msg);
                        //
                        var result = [txt, 0];
            ...
            function(callback){
                //check another level - intent id and value array result
                    //variable in lastResponse.intent.variables
                var var_id_res = [], var_value_res = [];
//                ptor.element.all(protractor.By.repeater('variable in lastResponse.intent.variables')).then(function(arr){
                ptor.element.all(protractor.By.repeater('variable in lastResponse.variables')).then(function(arr){
                    var i = 0;
                    async.whilst(
                        function(){ return i < arr.length; },
                        function(cb){
                            arr[i].evaluate('variable.id').then(function(var_id){
                                arr[i].evaluate('variable.value').then(function(var_value){
                                    var_id_res.push(var_id);
                                    var_value_res.push(var_value);
                                    //
                                    output_msg = '\nSelected intent id: ' + var_id;
                                    output_msg += '\nSelected intent variable value: ' + var_value;
                                    output_string += output_msg;
                                    logger.info(output_msg);
                                    //
                                    i++;
                        ...
                        function(err){
                            if(err){ //
                                ...
                                var result = [var_id_res, var_value_res];
                                callback(null, result);
                            }
                        }
                    );
            ...
            function(err, result){
                ...
                try{
                   ...
                   // expect(result[3][0]).to.match(/What\'s next/);
                    expect(result[3][0]).to.match(/\/liveassist\/app/);    //sign in page

                    expect(result[9][0].statusCode).to.eql(200);
                    expect(result[9][1]).to.match(/<state>agentPending<\/state>/);

                    expect(result[10][0]).to.match(/\/liveassist\/app/); //
                    expect(result[11][0]).to.match(/Waive fee/);                     //intent check

                    //agent selection output check
                    expect(result[14][0]).to.match(/WAIVE_FEE_REQUEST/); //intent id
                    expect(result[15][0][0]).to.match(/BILLING_MONTH/);
                    expect(result[15][1][0]).to.match(/2014-01/);

                    /*Note: Just 'WAVI_FEE_REQUEST intent click, no variables, so no valid following variables check anymore
                    //expect(result[16][0].toString()).to.match(/BILLING_MONTH/);
                    //expect(result[16][1][0]).to.match(/2014-01/);
                    //expect(result[16][1].toString()).to.match(/LATE_PAYMENT_REASON/);
                    //expect(result[18][1][1]).to.match(/null/);
                    */
