//TC1134: Verify that Live Assist agent GUI with valid agent user name and password but can not sign in as Adminstrator
//-------------------------------------------------------

describe('TC1134: Verify that Live Assist agent GUI with valid agent user name and password but can not sign in as Adminstrator', function() {
...
//global env settings
    ...
    this.timeout(35000); //set timeout for test case
    //
...
//-------------------------------------------------------
    it('TC1134: Verify that Live Assist agent GUI with valid agent user name and password but can not sign in as Adminstrator', function(done) {
...
*/
 ...
 /*           function(callback){
   ...
   */
            ...
            function(callback){
                //test on ng-click and button #####################
                element1 = ptor.element(protractor.By.id('adminSignInButton'));
                element1.click().then(function(){
            ...
            function(callback){
                var err_msg = [];
                output_msg = '';
                ptor.element.all(protractor.By.repeater('error in errors')).then(function(arr){
                    var i = 0;
                    async.whilst(
                        function(){ return i < arr.length; },
                        function(cb){
                            arr[i].evaluate('error').then(function(err){
                                err_msg.push(err);
                                //
                                output_msg += '\nLog in error message: ' + err;
                                output_string += output_msg;
                                console.log(output_msg);
                                //
                                i++;
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[8][0].toString()).to.match(/Error 401 Authentication Failed:/)
                        .and.to.match(/User \'TA0001\' is not allowed to log into mode \'admin\'/); //login page (all point to /app)
