//TC1204: Verify that Live Assist no agent timeout via simple IVR API method /inputInteractionStep and return useASRResult after timeout
//US7697
//-------------------------------------------------------
describe('TC1204: Verify that Live Assist no agent timeout via simple IVR API method /inputInteractionStep and return useASRResult after timeout', function() {
...
//-------------------------------------------------------
    it('TC1204: Verify that Live Assist no agent timeout via simple IVR API method /inputInteractionStep and return useASRResult after timeout', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    //logout all the agents if exist
                    logoutURL = 'https://' + TEST_HOST + ':8446' + '/liveassist/app/logout'; //special https request to logout agent
                    test_client.get(logoutURL, function(err, res, body) {
                        if (err) {
                            err_msg = 'Error found from http get request on: ' + logoutURL + err;
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[6][1]).to.match(/<state>agentPending<\/state>/);

                    expect(result[8][0].statusCode).to.eql(200);
                    expect(result[8][1]).to.match(/<inputInteractionStepResult>/)
                        .and.to.match(/<state>agentPending<\/state>/);

                    expect(result[10][0].statusCode).to.eql(200);
                    expect(result[10][1]).to.match(/<inputInteractionStepResult>/)
