//TC1191: Verify that Live Assist popup calendar when agent selecting date via book flight intent daily view with basic Home-Arrow(key)-Enter keyboard controls
//-------------------------------------------------------
describe('TC1191: Verify that Live Assist popup calendar when agent selecting date via book flight intent daily view with basic Home-Arrow(key)-Enter keyboard controls', function() {
//QA test setup ----------------------------------------------
    var case_ID = 1191;
...
//-------------------------------------------------------
    it('TC1191: Verify that Live Assist popup calendar when agent selecting date via book flight intent daily view with basic Home-Arrow(key)-Enter keyboard controls', function(done) {
        ...
        async.series([
                ...
                function(callback){
                    ...
                    ptor.driver.wait(function(){
                        ...
                        if(b){
                            ...
                            //press sign out button - robust
                            element1 = ptor.element(protractor.By.id('signOutButton'));
                            element1.sendKeys(protractor.Key.ENTER).then(function(){
                                output_msg = '\nsignOutButton button pressed';
                                output_string += output_msg;
                                console.log(output_msg);
                                //
                                callback(null, 10);
                ...
                function(callback){
                    ...
                    ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.TAB).then(function(){
                        ptor.driver.sleep(1000);
                        ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.TAB).then(function(){
                            ptor.driver.sleep(1000);
                            ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.TAB).then(function(){
                                ptor.driver.sleep(1000);
                                ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.ENTER).then(function () {
                                    ptor.driver.sleep(1000);
                                    // triggered popup calender
                                    ptor.driver.getAllWindowHandles().then(function (handles) {
                                        ...
                                        ptor.driver.switchTo().window(handles[0]).then(function () {
                                            ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.HOME).then(function () { //HOME key, first month of current year
                                                ptor.driver.sleep(500);
                                                ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.ARROW_RIGHT).then(function () { //Right key date + 1 month
                                                    ptor.driver.sleep(500);
                                                    ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.ARROW_RIGHT).then(function () { //Right key date + 1 month
                                                        ptor.driver.sleep(500);
                                                        ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.ARROW_LEFT).then(function () { //Left key date - 1 month
                                                            ptor.driver.sleep(500);
                ...
                function(callback){
                    //test with keyboard Ctrl + Enter (= 'Done' button press)
            ...
            function(err, result){
                ...
                try{
                    ...
                    //get test case expected year - month - day for assersion check
                    Date.prototype.e_yyyymmdd = function() {
                        var yyyy = this.getFullYear().toString();
                        var mm = (this.getMonth()+1).toString(); // getMonth() is zero-based
                        return yyyy + '-' + (mm[1]?mm:"0"+mm[0]) + '-' + '02'; // padding
                    };

                    d = new Date();
                    var expected_year_month_day = d.e_yyyymmdd();

                    var output_msg = '\nExpected test case date year-month-day for assertion check: ' + expected_year_month_day + '\n';
                    output_string += output_msg;
                    logger.info(output_msg);
                    //
                    ...
                    expect(result[17][1]).to.have.string(expected_year_month_day);  //verify if contain the basic keyboard control selected year-month
