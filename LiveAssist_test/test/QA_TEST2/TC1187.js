//TC1187: Verify that Live Assist popup calendar when agent select a date then ESC close with keyboard action
//US7411 As a Live Assist application developer I want to show a calendar popup to agents when selecting a date
//-------------------------------------------------------
describe('TC1187: Verify that Live Assist popup calendar when agent select a date then ESC close with keyboard action', function() {
...
//-------------------------------------------------------
    it('TC1187: Verify that Live Assist popup calendar when triggered by Enter then ESC to close with keyboard action', function(done) {
        ...
        async.series([
                                    ...
                                    element3.sendKeys(protractor.Key.ENTER).then(function(){          //trigger calendar popup calendar
                                        ...
                                        ptor.driver.getAllWindowHandles().then(function(handles) {
                                            ...
                                            ptor.driver.switchTo().window(handles[0]).then(function(){
                                                ptor.driver.sleep(1000);
                                                ptor.driver.switchTo().activeElement().sendKeys(protractor.Key.ESC).then(function () { //Enter key to confirm
                                                    ptor.driver.sleep(1000);
                                                    output_msg = '\nEnter key press\n';
                                                    output_string += output_msg;
                                                    logger.info(output_msg);
                ...
                function(callback){
                    ...
                    element1.sendKeys(protractor.Key.chord(protractor.Key.CONTROL, protractor.Key.ENTER), protractor.Key.NULL).then(function(){
                        output_msg = '\nDone - CTRL + ENTER';
                        output_string += output_msg;
                        logger.info(output_msg);
                        //
