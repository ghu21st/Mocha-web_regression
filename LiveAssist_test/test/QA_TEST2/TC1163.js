//TC1163: Verify that Live Assist agent GUI keyboard input CTRL+END to sign out from busy state after completing the request
//-------------------------------------------------------
describe('TC1163: Verify that Live Assist agent GUI keyboard input CTRL+END to sign out from busy state after completing the request', function() {
//QA test setup ----------------------------------------------
    var case_ID = 1163;
...
//-------------------------------------------------------
    it('TC1163: Verify that Live Assist agent GUI keyboard input CTRL+END to sign out from busy state after completing the request', function(done) {
        ...
        async.series([
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[12][0]).to.match(/Sign out by CTRL \+ END/); //login page (all point to /app)
                    expect(result[16][1]).to.match(/\"completionCause\":\"success\"/)
                        .and.to.match(/\"outcome\":/)
                        .and.to.match(/\"intent\":/)
                        .and.to.match(/\"id\":\"INTENT\"/)
                        .and.to.match(/\"value\":\"WAIVE_FEE_REQUEST\"/)
                        .and.to.match(/\"state\":\"useLiveAssistResult\"/);
                    expect(result[18][0]).to.match(/Agent sign in page and button found/); //login page (all point to /app)
