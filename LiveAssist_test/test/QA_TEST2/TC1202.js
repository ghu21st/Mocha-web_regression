//TC1202: Verify that Live Assist MRCP API with NLSML format settings get command grammar to return useASRResult and no escalation
//-------------------------------------------------------
describe('TC1202: Verify that Live Assist MRCP API with NLSML format settings get command grammar to return useASRResult and no escalation', function() {
...
//-------------------------------------------------------
    it('TC1202: Verify that Live Assist MRCP API with NLSML format settings get command grammar to return useASRResult and no escalation', function(done) {
...
// sessionStats=SessionStats[{CREATE=1401133560554, START_SESSION=1401133560554}],viewName=null
                ...
                function(callback){
                    ...
                    var testGrammar1 = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/data/vxmldemo/mrcpapi/callsteering/mainmenu.grxml';
                    var testGrammar2 = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/data/vxmldemo/mrcpapi/callsteering/operator.grxml';
                    var testBaseGrammarURI = [testGrammar1, testGrammar2]; //ex.:http://10.3.41.56:8080/liveassist/data/vxmldemo/mrcpapi/callsteering/mainmenu.grxml
                    ...
                    var testGrammarIds = ["session:0x00000001", "session:0x00000002"]; //alternative good one
                    //
                    var testBodyParam = JSON.stringify({
                        "sessionID": sessionID,
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[13][0].statusCode).to.eql(200);
                    expect(result[13][1].state).to.match(/useASRResult/);
