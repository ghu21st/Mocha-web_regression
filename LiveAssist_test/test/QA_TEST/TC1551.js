...
US3662: Implement Escalation Engine Interface
 TC1551: Verify activating an escalation classifier instance to be ConfidenceBasedEscalationClassifier
 **/
//QA test case description, verification & assertion check
describe("TC1501: Verify activating an escalation classifier instance to be ConfidenceBasedEscalationClassifier \n", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1551: Verify activating one of the instances  to be ConfidenceBasedEscalationClassifier\n", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                var testBodyParam = "";
                test_client.post(testBaseURL + '/activate/ConfidenceBasedEscalationClassifier', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='\n Error found from http post request on /activate/ConfidenceBasedEscalationClassifier' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 1);
                    }
