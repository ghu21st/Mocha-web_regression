/**
 ...
 TC1542: Verify call log when invoking a complete MRCP sip 
 **/
//QA test case description, verification & assertion check
describe("TC1542: Verify call log when invoking a complete MRCP API ", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1542: Verify call log when invoking a complete MRCP sip ", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                var testBodyParam = JSON.stringify({
                    ...
                    // externalSessionID: "0a0325e7_000047e1_52e050ba_00b1_0001", 
					 externalSessionID: randomExternalSessionID, 
            ...
            //callLog
                ...
                afterssionDate = new Date();
                logger.info(' afterssionDate getMinutes '+afterssionDate.getMinutes()+' \n');
			    logger.info(' afterssionDate getSeconds '+ afterssionDate.getSeconds() +' \n');
				
                    setTimeout(function(){
                        logger.info('\nwait 19 second for server\n'); //debugging
                        callback(null, 8);
                    }, 1000);
                }
				,
            ...
            function(err, result){
                ...
                try{
					...
					logger.info('CallLog content :', fileContent);
					
					
                    
					//expect(fileContent).to.match(/<\?xml.+\?>/);
                   expect(fileContent).to.have.string("LIVEASSIST");
					expect(fileContent).to.have.string("SchemaVersion=\"1.1\""); 
					expect(fileContent).to.have.string("Channel=\""+randomExternalSessionID+"\"");
					expect(fileContent).to.have.string("organizationID=\"Nuance\"");
					expect(fileContent).to.have.string("applicationID=\"MrcpApiCallSteering\"");
 
					expect(fileContent).to.have.string("START_SESSION");
					expect(fileContent).to.have.string("sessionID");
 					expect(fileContent).to.have.string("varName=\"Nuance-MrcpApiCallSteering\""); //varName="Nuance-MrcpApiCallSteering" 
                    expect(fileContent).to.have.string("varVersion=\"1.0\"");					
					expect(fileContent).to.have.string("from=\"4627\""); //ani="4627" 
					expect(fileContent).to.have.string("to=\"4627\""); //dnis="4627"
 
 
 
					expect(fileContent).to.have.string("INTERACTION"); 
					expect(fileContent).to.have.string("context=\"mainmenu\"");
					expect(fileContent).to.have.string("pickList=\"nuance\/mrcpapicallsteering\/intents.json\"");

					
					expect(fileContent).to.have.string("SEND_INPUT"); 
					 expect(fileContent).to.have.string("text=\"i would like to book a flight\"");
					 expect(fileContent).to.have.string("confidence=\"0.78\"");
					expect(fileContent).to.have.string("duration=\"3240\"");
					expect(fileContent).to.have.string("eos=\"850\"");
                    expect(fileContent).to.have.string("utterance=\"http://mtl-da53/vxml/NUAN-22-24-mtl-da53-0a0325e7_000011c2_5307b580_00d4_0001-utt001-SAVEWAVEFORM.wav\"");
