/**
US3790 escalation engine wraper
 TC1532:  Escalation Engine Wrapper - Verify that a newly uploaded classifier that throws an exception does not crash the VXML service, then remove the instance
 **/
//QA test case description, verification & assertion check0
describe("TC1532:   Escalation Engine Wrapper - Verify that a newly uploaded classifier that throws an exception does not crash the VXML service, then remove the instance", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1532 :  Verify Uploading a new escalation classifier instance from groovy type with exception then remove it\n", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                test_client.put(testBaseURL + '/TimeBasedEscalationClassifier' , testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http put  request on /sessions' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 1);
                    }
