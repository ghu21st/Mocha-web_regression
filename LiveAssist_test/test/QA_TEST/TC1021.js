/**
 TC1021: Verify that system https response when sending interaction result with invalid session id 
 **/
//QA test case description, verification & assertion check
describe("TC1021: Verify that system https response when sending interaction result with invalid session id", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1021: Verify that system https response when sending interaction result with invalid session id", function (done) {
        ...
        async.series([
                ...
                function(callback){
                    ...
                    var output_msg = '';
                    var testBodyParam = JSON.stringify({
                        sessionID: "session:NINASMSTEST:12341234-1234-1234-1234-123412341234" //invalid session ID for this case
                    });
                    test_client.post2(testAPIURL + '/inputInteractionStep', testBodyParam, function(err, res, body){
                        /*if(err){
                            err_msg='Error found from http post request on /inputInteractionStep' + err;
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[1][1].state).to.match(/agentPending/);

                    //verify the case result
                    expect(result[3][0].statusCode).to.eql(404);
                    expect(result[3][0].body).to.have.string('Can\'t find session with sessionID');
