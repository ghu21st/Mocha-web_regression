...
US3662: Implement Escalation Engine Interface
 TC1555: Verify activating an escalation classifier instance to be NeverEscalatingEscalationClassifier
 **/
//QA test case description, verification & assertion check
describe("TC1555: Verify activating an escalation classifier instance to be NeverEscalatingEscalationClassifier \n", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1555: Verify activating one of the instances  to be NeverEscalatingEscalationClassifier\n", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                var testBodyParam = "";
                test_client.post(testBaseURL + '/activate/NeverEscalatingEscalationClassifier', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='\n Error found from http post request on /activate/NeverEscalatingEscalationClassifier' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 1);
                    }
            ...
            function(err, result){
                ...
                try{
					...
					expect(result[1][1]).to.match(/\"name\":\"NeverEscalatingEscalationClassifier\"/);
					expect(result[1][1]).to.match(/\"implementationClassName\":\"com.nuance.liveassist.server.escalation.NeverEscalatingEscalationClassifier\"/);
