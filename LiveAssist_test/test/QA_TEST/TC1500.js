/**
DE2354 "source" property is listed now at the end of each classifier (It was not the case previously)
DE1137
US3662: Implement Escalation Engine Interface

 TC1500: Verify listing all escalation classifier instance

 **/
//QA test case description, verification & assertion check
describe("TC1500: Verify listing all escalation classifier instance\n", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1500: Verify listing all escalation classifier instance", function (done) {
        ...
        async.series([
            function(callback){
                //special timeout here to make sure it disconnect
                setTimeout(function(){
            ...
            function(err, result){
                ...
                try{
					...
					expect(result[2][1]).to.match(/\"ConfidenceBasedEscalationClassifier\"/);
					expect(result[2][1]).to.match(/\"AlwaysEscalatingEscalationClassifier\"/);
					expect(result[2][1]).to.match(/\"PercentageBasedEscalationClassifier\"/);
					expect(result[2][1]).to.match(/\"NeverEscalatingEscalationClassifier\"/);
					expect(result[2][1]).to.match(/\"DynamicEscalationClassifier\"/);
				

				    //expect(result[2][0].Content-Type).to.eql('application/json');
                    //Verify with details
					
                    expect(result[2][1]).to.match(/"name":"AlwaysEscalatingEscalationClassifier","implementationClassName":"com.nuance.liveassist.server.escalation.AlwaysEscalatingEscalationClassifier","source":"PrePackaged","active":true,"properties":{}/);
					
					expect(result[2][1]).to.match(/"name":"PercentageBasedEscalationClassifier","implementationClassName":"com.nuance.liveassist.server.escalation.PercentageBasedEscalationClassifier","source":"PrePackaged","active":false,"properties":{"percentage":50.0}/);
					
					expect(result[2][1]).to.match(/"name":"NeverEscalatingEscalationClassifier","implementationClassName":"com.nuance.liveassist.server.escalation.NeverEscalatingEscalationClassifier","source":"PrePackaged","active":false,"properties":{}/);
					
					expect(result[2][1]).to.match(/"ConfidenceBasedEscalationClassifier","implementationClassName":"com.nuance.liveassist.server.escalation.ConfidenceBasedEscalationClassifier","source":"PrePackaged","active":false,"properties":{"confidenceThreshold":/);//"0.5"}/);
					
					expect(result[2][1]).to.match(/"DynamicEscalationClassifier","implementationClassName":"com.nuance.liveassist.server.escalation.DynamicEscalationClassifier","source":"PrePackaged","active":false,"properties":{"confidenceThreshold":/);
