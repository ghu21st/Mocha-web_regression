/**
Can not be tested till DE1135 is fixed.
US3770: Support Escalation Engine Properties
 TC1506:  Verify US3770 Support escalation engine properties 
 http://10.3.41.61:8080/liveassist/rest/escalationClassifiers/PercentageBasedEscalationClassifier/setProperty/percentage/50
 **/
//QA test case description, verification & assertion check0
describe("TC1506:   Verify updating configuration of managed instances", function () {
    //QA test setup ----------------------------------------------
	...
	// var activatePercentageBasedEscalationClassifier  = require('/QA_TEST/PercentageBased/TC1554.js');
    //---------------------------------------------
    ...
    before(function(done1){
        ...
        //
		//mocha --reporter spec --ui bdd PercentageBased/ --timeout 3600s --slow 60000
        done1();
    });
...
//                            console.log('\ndeleted module:' + key);
            ...
            console.log('Test elapsed=' + test_elapse);
            //
			//mocha --reporter spec --ui bdd AlwaysEscalating/ --timeout 3600s --slow 60000
            done2();
        });
    });


    it("TC1506:  updating configuration of managed instances", function (done) {
        ...
        async.series([
			...
			function(callback){
                ...
                var testBodyParam = "";
                test_client.post(testBaseURL + '/activate/PercentageBasedEscalationClassifier', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='\n Error found from http post request on /activate/PercentageBasedEscalationClassifier' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 1);
                    }
                    output_string += '\n\nDebug: post request to activate an instance is done\n\n';//for debugging
            ...
            function(callback){
                //change value to 75
	// var activateAlwaysEscalatingEscalationClassifier  = require('/AlwaysEscalating/TC1503.js');
                var testBodyParam = "";
                test_client.post(testBaseURL + '/PercentageBasedEscalationClassifier/setProperty/percentage/75' , testBodyParam, function(err, res, body){
                    if(err){
	...
	//back to default value
                var testBodyParam = "";
                test_client.post(testBaseURL + '/PercentageBasedEscalationClassifier/setProperty/percentage/50' , testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http delete request on /sessions' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
			...
			function(callback){
                //Debug API - delete all sessions (status: 204)
                var testBodyParam = "";
                test_client.post(testBaseURL + '/activate/AlwaysEscalatingEscalationClassifier', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='\n Error found from http post request on /activate/AlwaysEscalatingEscalationClassifier' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 4);
                    }
            ...
            function(err, result){
                ...
                try{
					...
					expect(result[1][1]).to.match(/\"name\":\"PercentageBasedEscalationClassifier\"/);
					expect(result[1][1]).to.match(/\"implementationClassName\":\"com.nuance.liveassist.server.escalation.PercentageBasedEscalationClassifier\"/);
					expect(result[1][1]).to.match(/\"source\":\"PrePackaged\"/);
					expect(result[1][1]).to.match(/\"active\":true/);
					expect(result[1][1]).to.match(/\"properties\":{\"percentage\":50.0}/);
					

					expect(result[2][0].statusCode).to.eql(200);
					expect(result[2][1]).to.match(/\"name\":\"PercentageBasedEscalationClassifier\"/);
					...
					expect(result[2][1]).to.match(/\"properties\":{\"percentage\":75.0}/);

					
					expect(result[3][0].statusCode).to.eql(200);
					expect(result[3][1]).to.match(/\"name\":\"PercentageBasedEscalationClassifier\"/);
					expect(result[3][1]).to.match(/\"implementationClassName\":\"com.nuance.liveassist.server.escalation.PercentageBasedEscalationClassifier\"/);
					expect(result[3][1]).to.match(/\"source\":\"PrePackaged\"/);
					expect(result[3][1]).to.match(/\"active\":true/);
					expect(result[3][1]).to.match(/\"properties\":{\"percentage\":50.0}/);
