/**
 TC1502: Getting details about a particular escalationClassifier instance

 **/
//QA test case description, verification & assertion check
describe("TC1502: Getting details about a particular escalationClassifier instance\n", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1502: Verify get details about AlwaysEscalatingEscalationClassifier - NeverEscalatingEscalationClassifier - PercentageBasedEscalationClassifier", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                var testBodyParam = "";
                test_client.get(testBaseURL + '/AlwaysEscalatingEscalationClassifier', function(err, res, body){
                    if(err){
			 ...
			 function(callback){
                //Debug API - delete all sessions (status: 204)
                var testBodyParam = "";
                test_client.get(testBaseURL + '/NeverEscalatingEscalationClassifier', function(err, res, body){
                    if(err){
                        err_msg='\n Error found from http get request on escalationClassifiers' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
            ...
            function(err, result){
                ...
                try{
					...
					expect(result[1][1]).to.match(/\"name\":\"AlwaysEscalatingEscalationClassifier\"/)
					.and.to.match(/\"implementationClassName\":\"com.nuance.liveassist.server.escalation.AlwaysEscalatingEscalationClassifier\"/)
					.and.to.match(/\"source\":\"PrePackaged\"/)
					.and.to.match(/\"active\":true/)
					.and.to.match(/\"properties\":{}/);
					
					expect(result[2][0].statusCode).to.eql(200);	
					expect(result[2][1]).to.match(/\"name\":\"NeverEscalatingEscalationClassifier\"/)
					.and.to.match(/\"implementationClassName\":\"com.nuance.liveassist.server.escalation.NeverEscalatingEscalationClassifier\"/)
					.and.to.match(/\"source\":\"PrePackaged\"/)
					.and.to.match(/\"active\":false/)
					.and.to.match(/\"properties\":{}/);
					
					expect(result[3][0].statusCode).to.eql(200);	
					expect(result[3][1]).to.match(/\"name\":\"PercentageBasedEscalationClassifier\"/)
					.and.to.match(/\"implementationClassName\":\"com.nuance.liveassist.server.escalation.PercentageBasedEscalationClassifier\"/)
					.and.to.match(/\"source\":\"PrePackaged\"/)
					.and.to.match(/\"active\":false/)
					.and.to.match(/\"properties\":{"percentage":50.0}/);
