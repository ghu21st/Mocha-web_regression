...
US3662: Implement Escalation Engine Interface
 TC1559: Getting details about a particular escalationClassifier instance that is PrePackaged - i.e.PercentageBasedEscalationClassifier

 **/
//QA test case description, verification & assertion check
describe("TC1559: Getting details about PercentageBasedEscalationClassifier\n", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1559 : Verify Getting details about PercentageBasedEscalationClassifier", function (done) {
        ...
        async.series([
			...
			function(callback){
                var testBodyParam = "";
                test_client.get(testBaseURL + '/PercentageBasedEscalationClassifier' , function(err, res, body){
                    if(err){
            ...
            function(err, result){
                ...
                try{
					...
					expect(result[1][1]).to.match(/\"source\":\"PrePackaged\"/);
					// expect(result[1][1]).to.match(/\"active\":false/);
					// expect(result[1][1]).to.match(/\"properties\":{\"confidenceThreshold\":\"0.5\"}/);
					

					 expect(result[2][0].statusCode).to.eql(200);
				    //expect(result[2][0].Content-Type).to.eql('application/json');
                    //TODO: To update the validation of the response after the fixing the order of the list
					...
					// expect(result[3][1]).to.match(/\"active\":false/);
					...
					// expect(result[3][1]).to.match(/\"properties\":{}/);
				*/	
