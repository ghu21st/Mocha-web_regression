/**
DE829: Error found in Uploading a new escalation classifier with Content-Type GROOVY that does not compile
US3662: Implement Escalation Engine Interface
 TC1509:  Uploading a new escalation classifier instance when the bytes contained in the body do not represent a valid instance of the specified type (not java archive for example)
 **/
//QA test case description, verification & assertion check0
describe("TC1509:   Verify Uploading a new escalation classifier instance when the bytes contained in the body do not represent a valid instance of the specified type", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1509:  Verify Uploading a new escalation classifier instance from groovy type but with compilation errors in the content of the body", function (done) {
        ...
        async.series([
            ...
            function(err, result){
                ...
                try{
                    ...
                    //TODO DE829
                    expect(result[1][0].statusCode).to.eql(400);
					// expect(result[1][1]).to.match(/Unable to handle \'Escalation Classifiers\' instances in files of type \'application\/json\'/);
				
					//make sure that delete return that the instance is not exists
					expect(result[2][0].statusCode).to.eql(204);
					//expect(result[2][1]).to.match(/No 'interface com.nuance.liveassist.escalation.IEscalationClassifier' instance with name 'TimeBasedEscalationClassifier' exists/);
                    ...
                    //
                    var fail_error = '\n\nTest case ' + case_ID + ' failed! Please check case log for details';
