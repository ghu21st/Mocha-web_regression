/**
	DE928 (fixed by changing the documenation)
 US3269: Implement confidence based routing
 TC1524: Verify Invalid confidence thresholds
 Setting the confidence threshold of the escalation classifier to a value other than a real number between 0.0 and 1.0 inclusive should result in an error.
 If doing so via an admin service, it should result in a 400 Bad Request error 

 **/
//QA test case description, verification & assertion check
describe("TC1524: Verify Invalid confidence thresholds \n", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1524 : Verify updating configuration of managed instances of ConfidenceBasedEscalationClassifier for invalid value of confidenceThreshold .e.g. equal 3 then equal to -1\n", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                var testBodyParam = "";
                test_client.post(testBaseURL + '/ConfidenceBasedEscalationClassifier/setProperty/confidenceThreshold/3.0' , testBodyParam, function(err, res, body){
                    if(err){
			 ...
			 function(callback){
                //Debug API - delete all sessions (status: 204)
                var testBodyParam = "";
                test_client.post(testBaseURL + '/ConfidenceBasedEscalationClassifier/setProperty/confidenceThreshold/-1' , testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http delete request on /sessions' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
            ...
            function(err, result){
                ...
                try{
                    ...
                    // expect(result[1][1]).to.match(/Raw confidence cannot be greater than 1.0/);
