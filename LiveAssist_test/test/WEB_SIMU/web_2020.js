/*
 web_2020: Web GUI test simulator - agent response outcome result with App1: book-flight, App2: Boston to Montreal on Tuesday
 */
...
async.series([
    ...
    //2. App1 agent Web Test Driver - outcome book-flight --------------------------------------------------
    function(callback){
        //start web test driver based on app driver session
        //set base agent web page URL
    ...
    function(callback){
        ...
        var element1 = driver.findElement(webdriver.By.xpath("//input[@id='pickListSelection1']"));
        if(element1.isDisplayed()){
            element1.click();
    ...
    //3. App2: Agent Web Test Driver - Travel app outcome --------------------------------------------------
    function(callback){
        //start web test driver based on app driver session
        //set base agent web page URL
        var BaseAgentURL = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/agent/sessionScreen?sessionId=' + sessionID;
        ...
        driver.get(BaseAgentURL);
            output_msg = '\nQA Web test Driver ---- \nDebug: Test base URL ' + BaseAppAPIURL;//for debugging
            output_string += output_msg;
            console.log(output_msg);
            callback(null, 12);
    ...
    function(callback){
        ...
        driver.wait(function() {
            return driver.isElementPresent(webdriver.By.id('outcomeForm'));
        }, 22000).then(function(b){
                if(b){
                    output_msg = '\nFound the outcome selection agent web page with specific session id';//for debugging
                    output_string += output_msg;
                    console.log(output_msg);
    ...
    function(callback){
        // Customize click/select one of agent selections
        var select = driver.findElement(webdriver.By.name("outcomeValues['originCity']"));
        select.sendKeys("Boston").then(function(){
//                var option = select.findElement(webdriver.By.xpath("//option[@value='Boston']"));
//               option.click().then(function(){
            output_msg = '\nSelect1 - Option - clicked!';//for debugging
            output_string += output_msg;
            console.log(output_msg);
            callback(null, 14);
        });
    ...
    function(callback){
        // Customize click/select one of agent selections
        var select = driver.findElement(webdriver.By.name("outcomeValues['destinationCity']"));
        select.sendKeys("Montreal").then(function(){
//                var option = select.findElement(webdriver.By.xpath("//option[@value='Montreal']"));
//                option.click().then(function(){
            output_msg = '\nSelect2 - Option - clicked!';//for debugging
            output_string += output_msg;
            console.log(output_msg);
            callback(null, 15);
        });
    ...
    function(callback){
        // Customize click/select one of agent selections
        var select = driver.findElement(webdriver.By.name("outcomeValues['departureDay']"));
        select.sendKeys("Tuesday").then(function(){
//                var option = select.findElement(webdriver.By.xpath("//option[@value='Tuesday']"));
//                option.click().then(function(){
            output_msg = '\nSelect3 - Option - clicked!';//for debugging
            output_string += output_msg;
            console.log(output_msg);
            callback(null, 16);
    ...
    function(callback){
        ...
        var element1 = driver.findElement(webdriver.By.name("outcomeValues['originCity']"));
        var element2 = driver.findElement(webdriver.By.name("outcomeValues['destinationCity']"));
        var element3 = driver.findElement(webdriver.By.name("outcomeValues['departureDay']"));
        element1.isSelected().then(function(b){
            if(b){
                output_msg = '\nFound the correct agent web page select1 element';//for debugging
                output_string += output_msg;
                console.log(output_msg);

                element2.isSelected().then(function(b2){
                    if(b2){
                        output_msg = '\nFound the correct agent web page select2 element';//for debugging
                        output_string += output_msg;
                        console.log(output_msg);

                        element3.isSelected().then(function(b3){
                            if(b3){
                                output_msg = '\nFound the correct agent web page select3 element';//for debugging
                                output_string += output_msg;
                                console.log(output_msg);
                                callback(null, 17);
                            }else{
                                console.log('\n ERROR! - Can not find the correct agent outcome selection3 result');
                                output_string += output_msg;
                                console.log(output_msg);
                        ...
                        console.log('\n ERROR! - Can not find the correct agent outcome selection2 result');
                        output_string += output_msg;
                        console.log(output_msg);
                ...
                console.log('\n ERROR! - Can not find the correct agent outcome selection1 result');
                output_string += output_msg;
                console.log(output_msg);
    ...
    function(callback){
        //submit simulated 'Agent' outcome result selection
        var element = driver.findElement(webdriver.By.id('outcomeForm'));
        element.submit().then(function(){
            output_msg = '\nSubmit simulated Agent outcome result selection';//for debugging
            output_string += output_msg;
            console.log(output_msg);
    ...
    //4. Back to QA App Test Driver to check result ---------------------------------------------
...
*/
  ...
  /*          //http response return assertion check
            ...
            if(result[3][1].search(sessionID) == -1){
